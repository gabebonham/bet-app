...
def prever_proximos_resultados(matriz, modelo, scaler, janela_tempo=24, hora_inicial=19, horizonte=5):
    num_linhas, num_colunas = matriz.shape
    ...
    # Ajustar para começar da hora_inicial
    # Assumindo que a última linha da matriz é a hora atual (19)
    # e queremos prever as próximas 5 horas (20, 21, 22, 23, 00)
    
    for h in range(horizonte):
        for j in range(num_colunas):
            # Extrair janela de tempo atual
            i = num_linhas - janela_tempo
...
# Prever próximos resultados (próximas 5 horas a partir da hora 19)
print("\nRealizando previsões para as próximas 5 horas a partir da hora 19...")
previsoes, probabilidades = prever_proximos_resultados(matriz_dados, rf_model, scaler, janela_tempo=24, hora_inicial=19, horizonte=5)
...
plt.title('Previsões para as Próximas 5 Horas (a partir da hora 19)')
plt.xlabel('Colunas (Campeonatos)')
plt.ylabel('Horas Futuras (20, 21, 22, 23, 00)')
...
plt.title('Probabilidades de BTTS para as Próximas 5 Horas (a partir da hora 19)')
plt.xlabel('Colunas (Campeonatos)')
plt.ylabel('Horas Futuras (20, 21, 22, 23, 00)')
...
plt.tight_layout()
plt.savefig(f"{output_dir}/previsoes_5_horas_a_partir_19.png", dpi=300, bbox_inches='tight')

# Criar tabela de recomendações para as próximas 5 horas
recomendacoes = []
horas_futuras = [20, 21, 22, 23, 0]  # Horas 20, 21, 22, 23, 00

for idx, hora in enumerate(horas_futuras):
    # Ordenar campeonatos por probabilidade para esta hora
    campeonatos_prob = [(j, probabilidades[idx, j]) for j in range(probabilidades.shape[1])]
...
# Mapear números de campeonatos para nomes
def mapear_campeonato(coluna):
    if coluna in [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 52, 55, 58]:
        prefixo = "COPA"
    elif coluna in [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59]:
        prefixo = "EURO"
    elif coluna in [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 52, 55, 58]:
        prefixo = "SUPER"
    else:
        prefixo = "PREMIER"
    
    return f"{prefixo} {coluna:02d}"

# Salvar recomendações em arquivo de texto
with open(f"{output_dir}/recomendacoes_5_horas_a_partir_19.txt", 'w') as f:
    f.write("RECOMENDAÇÕES PARA AS PRÓXIMAS 5 HORAS A PARTIR DA HORA 19 (ESTRATÉGIA MARTINGALE)\n")
    f.write("==========================================================================\n\n")
    ...
    for rec in recomendacoes:
        ...
        for i, (campeonato, prob) in enumerate(rec['melhores_campeonatos']):
            nome_campeonato = mapear_campeonato(campeonato)
            f.write(f"Opção {i+1}: Campeonato {campeonato} ({nome_campeonato}) - Probabilidade: {prob:.4f}\n")
...
# Criar arquivo de previsões no formato da tabela original
with open(f"{output_dir}/previsoes_formato_tabela_original_a_partir_19.txt", 'w') as f:
    f.write("PREVISÕES BTTS PARA AS PRÓXIMAS 5 HORAS (A PARTIR DA HORA 19) - FORMATO TABELA ORIGINAL\n\n")
    f.write("HORA | CAMPEONATO | COLUNA | MERCADO | PROBABILIDADE | CONFIANÇA\n")
    f.write("-----|------------|--------|---------|--------------|----------\n")
    ...
    for idx, hora in enumerate(horas_futuras):
        ...
        # Selecionar os 3 melhores campeonatos para esta hora
        for i, (campeonato, prob) in enumerate(campeonatos_prob[:3]):
            # Determinar o tipo de campeonato e seu número formatado
            if campeonato in [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 52, 55, 58]:
                tipo_campeonato = "COPA"
            elif campeonato in [2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, 38, 41, 44, 47, 50, 53, 56, 59]:
                tipo_campeonato = "EURO"
            elif campeonato in [1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, 37, 40, 43, 46, 49, 52, 55, 58]:
                tipo_campeonato = "SUPER"
            else:
                tipo_campeonato = "PREMIER"
            ...
            f.write(f"{hora:02d}   | {tipo_campeonato} {campeonato:02d} | {campeonato:2d}     | BTTS    | {prob*100:.2f}%       | {confianca}\n")
    ...
    f.write("INSTRUÇÕES PARA USO:\n")
    f.write("1. A coluna HORA indica a hora na tabela (lado esquerdo)\n")
    f.write("2. A coluna CAMPEONATO indica o nome e número do campeonato\n")
    f.write("3. A coluna COLUNA indica o número da coluna na tabela\n")
    f.write("4. Todas as previsões são para o mercado BTTS (Both Teams To Score)\n")
    f.write("5. As previsões estão ordenadas por hora e probabilidade (da maior para a menor)\n\n")
    
    f.write("ESTRATÉGIA MARTINGALE RECOMENDADA:\n")
    f.write("1. Para cada hora, comece apostando no campeonato com maior probabilidade\n")
    f.write("2. Se perder, passe para o segundo campeonato com maior probabilidade\n")
    f.write("3. Se perder novamente, passe para o terceiro campeonato com maior probabilidade\n")

print(f"Previsões e recomendações para as próximas 5 horas a partir da hora 19 salvas em: {output_dir}")
