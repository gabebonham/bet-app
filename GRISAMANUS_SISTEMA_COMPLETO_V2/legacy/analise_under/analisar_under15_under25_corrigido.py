...
def load_data(self, filepath):
    """Load and preprocess raw match data"""
    df = pd.read_csv(filepath)
    
    # Convert and extract time components
    df['HORA'] = pd.to_datetime(df['Tempo']).dt.hour
    df['MINUTO'] = pd.to_datetime(df['Tempo']).dt.minute
    
    # Add time cycle categorization
    df['CICLO'] = df['HORA'].apply(
        lambda h: next((k for k,v in self.time_cycles.items() if h in v), 'DESCONHECIDO')
    )
    
    # Calculate goal totals
    df['TOTAL_GOLS'] = df['Gols time casa'] + df['Gols time contra']
...
def calcular_correlacao_under15_0x0():
    ...
    df_combined = pd.DataFrame(dados_combinados)
